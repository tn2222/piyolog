...
import {
  HttpPiyologDataFeedClient,
  PiyologDataFeedClientError,
} from "../src/infrastructure/externalService/piyologDataFeedClient";
...
describe("HttpPiyologDataFeedClient", () => {
  it("fetches and validates a snapshot", async () => {
    ...
    await expect(client.getDataFeed()).resolves.toMatchObject({
  ...
  it("retries 429 and 5xx responses with injectable backoff", async () => {
    ...
    await expect(client.getDataFeed()).resolves.toMatchObject({ records: [{ eventId: "event-1" }] });
  ...
  it("does not retry non-retryable HTTP errors", async () => {
    ...
    const sleepMock = vi.fn(async () => {});
    const client = new HttpPiyologDataFeedClient({ url, fetch: fetchMock, sleep: sleepMock });

    await expect(client.getDataFeed()).rejects.toMatchObject({
      name: "PiyologDataFeedClientError",
      code: "http_error",
      status: 404,
    });
  ...
  it("does not expose the configured URL in request errors", async () => {
    ...
    const client = new HttpPiyologDataFeedClient({ url, fetch: fetchMock, maxAttempts: 1 });

    const error = await client.getDataFeed().catch((value: unknown) => value);
    expect(error).toBeInstanceOf(PiyologDataFeedClientError);
