...
5分ごとに同じファイル名の新規ファイルが作られる運用でも、Script Properties は増え続けません。フォルダ内の最新テキストファイル1件を処理対象にし、前回処理した最新ファイルと同じなら送信しません。新しいファイルが追加された場合、または最新ファイルが更新された場合は Worker に送信します。

取り込みに成功した後、または最新ファイルがすでに処理済みだった場合、同じフォルダ内の古いテキストファイルは Google Drive のゴミ箱へ移動します。最新ファイルは次回の処理済み判定に使うため残します。
