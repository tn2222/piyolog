...
### GitHub Actions デプロイの前提

GitHub Actions からデプロイする場合は、Cloudflare の公式 GitHub Actions 手順に従い、CI/CD 用の認証情報を GitHub repository secrets として設定します。
...
workflow の起動条件は、通常運用では `main` ブランチへの push でデプロイし、必要に応じて `workflow_dispatch` で手動実行できるようにします。deploy job では Node.js 24.14.1 を使い、`npm ci`、`npm run typecheck`、`npm test` の成功後に Cloudflare 公式の `cloudflare/wrangler-action` でデプロイします。
